...
const AimlHiring = () => {
  const cards = [
    ...
    },
    {
      ...
      features: ['4+ Years Senior Developer', '160 hours/month', 'Dedicated resource'],
      buttonText: 'Get Started',
    },
    {
      id: 'fixed',
      ...
      features: ['Timely Delivery', 'Within Budget', 'Complete scoping & planning'],
  ...
  return (
    <div className="bg-white py-16 px-4 sm:px-8 md:px-16 max-w-full mx-auto">
      <h2 className="text-2xl md:text-4xl font-bold text-center mb-2">
        Hire Custom Software Developer As Per Your Need
      </h2>
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-6 md:gap-8">
        {cards.map((card) => (
          <div
            key={card.id}
            className="w-full md:w-80 bg-white rounded-lg shadow-lg border border-gray-200 p-6 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:z-10 relative mb-6 md:mb-0"
          >
            <div className={`text-center ${card.highlightText ? 'pt-8' : ''}`}>
              <div className="text-4xl font-bold text-orange-500">
                {card.price}
                <span className="text-xl">{card.note}</span>
              </div>
